...
// ====== NEBULA - awan gas tipis, partikel sangat sedikit ======
function createNebula(): THREE.Points {
  const count = 80; // sangat sedikit
...
// ====== DEBRIS - 1-2 pieces saja ======
function createDebris(): THREE.Group {
  const group = new THREE.Group();
  const count = 1 + Math.floor(Math.random() * 2); // max 2
...
// ====== KOMAT - tanpa ekor partikel, hanya inti ======
...
// ====== Hitung radius bounding untuk collision ======
...
export default function SpaceEnvironment() {
  ...
  useEffect(() => {
    ...
    // FOG TEBAL - batasi jarak pandang
    ...
    // ====== BINTANG BACKGROUND (ikut kamera, tidak pernah tabrakan) ======
    ...
    interface SpaceObj {
      mesh: THREE.Object3D;
      type: string;
      radius: number; // bounding radius untuk collision
    ...
    function spawnObject(): boolean {
      ...
      // Cari posisi spawn yang aman (max 15 percobaan)
      let placed = false;
      ...
      for (let attempt = 0; attempt < 15; attempt++) {
        ...
        spawnPos = camera.position.clone()
          .add(forward.clone().multiplyScalar(dist * 0.6))
      ...
      if (!placed) {
        // Cleanup mesh yang gagal di-place
        mesh.traverse((child) => {
      ...
      objects.push({ mesh, type, radius, rotSpeed, driftVel });
      return true;
    }
    ...
    const animate = () => {
        ...
        if (dist < FADE_START) {
          ...
          obj.mesh.traverse((child) => {
            if (child instanceof THREE.Mesh || child instanceof THREE.Points) {
              ...
              const base = obj.type === 'nebula' ? 0.15 : 1.0;
              mat.transparent = true;
              mat.opacity = base;
            }
          });
        } else if (dist < FADE_END) {
          // Fade out gradual ke dalam kabut
          const t = (dist - FADE_START) / (FADE_END - FADE_START);
          const opacity = 1 - t * t; // easing kuadratik
          obj.mesh.traverse((child) => {
            if (child instanceof THREE.Mesh || child instanceof THREE.Points) {
              const mat = child.material as THREE.Material & { opacity: number; transparent: boolean };
              const base = obj.type === 'nebula' ? 0.15 : 1.0;
      ...
      // ====== SPAWN LOOP - terus menerus ======
      spawnTimer += delta;
      if (objects.length < MAX_OBJECTS && spawnTimer > 1.5) {
        spawnObject();
        spawnTimer = 0;
      }
    ...
    <div style={{ width: '100vw', height: '100vh', overflow: 'hidden', position: 'relative', background: '#000' }}>
      ...
      {loading && (
        <div style={{
          ...
          <div style={{ color: '#445', fontSize: '16px', fontFamily: 'monospace' }}>
            Memuat...
          </div>
        </div>
      )}
